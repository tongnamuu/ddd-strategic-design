...
#### 상품
* 상품은 이름과 가격을 갖는다
  * 이름은 반드시 존재하며 비속어를 포함할 수 없다
  * 가격은 0 이상이다
* 사업자는 판매하려는 상품을 등록한다
* 사업자는 상품의 가격을 변경할 수 있다
* 상품이 가격이 변경될 때는 해당 상품을 포함하는 메뉴의 전시여부에 영향을 미친다
  * 메뉴의 가격이 (메뉴에 포함된 제품들의 가격 * 수량)의 합계보다 크다면 메뉴는 숨겨진다
* 사업자는 상품 목록을 조회한다 
* 하나의 상품(product)은 메뉴상품(MenuProduct) 에 포함될 수 있다

#### 메뉴
* 메뉴는 메뉴상품을 가지며 메뉴이름, 메뉴가격, 전시여부를 갖고 있다
  * 가격은 0 이상이다
  * 이름은 반드시 존재하며 비속어를 포함할 수 없다
    * 비속어 확인은 별도 외부서비스를 활용한다
  * 메뉴그룹에 포함될 수 있다.
    * 메뉴그룹에 포함되지 않는 메뉴는 저장할 수 없다
* 메뉴의 가격을 변경할 수 있다
  * 가격은 0 이상이다
* 메뉴의 전시상태를 변경할 수 있다
...
* 배달주문
  ...
  * 주문상태
    * 여러개의 주문상품(OrderItem) 을 가진 주문이 대기 상태로 만들어진다
      * 주문상품은 하나의 메뉴와 수량 정보를 갖는다. 
    * 사업자가 대기상태의 주문을 수락하여 수락상태로 변경한다
    * 수락 상태의 주문을 서빙상태로 변경하고 주문된 메뉴를 준비한다
    * 메뉴가 준비되면 외부 배달대행사를 통해 배달 시작 상태로 변경한다
      * 배달대행사 서비스로는 주문된 메뉴들의 (가격과 수량의 곱) 의 합과 주문식별자, 배달주소를 전달한다
    * 배달이 완료되면 배달완료로 변경된다
    * 이후에 주문완료로 변경된다.
* 매장식사주문
  * 주문매장테이블(OrderTable) 이 반드시 필요하다.
    * 주문매장테이블의 상태를 사용중(sit) 으로 변경할 수 있다
    * 주문매장테이블은 정리할 수 있다
      * 정리하고 나면 미사용상태로 변하고 손님수는 0이다
    * 사용중인 테이블의 경우 손님수를 변경할 수 있으며 0이상이다
  * 주문상태
    * 여러개의 주문상품(OrderItem) 을 가진 주문이 대기 상태로 만들어진다
      * 주문상품은 하나의 메뉴와 수량 정보를 갖는다.
    * 사업자가 대기상태의 주문을 수락하여 수락상태로 변경한다
    * 수락 상태의 주문을 서빙상태로 변경하고 주문된 메뉴를 준비한다
    * 주문이 서빙된 이후 주문완료로 변경된다.
* 포장식사주문
  * 주문상태
    * 여러개의 주문상품(OrderItem) 을 가진 주문이 대기 상태로 만들어진다
      * 주문상품은 하나의 메뉴와 수량 정보를 갖는다.
